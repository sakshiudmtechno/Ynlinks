'use client';

import {
  Facebook,
  Instagram,
  Linkedin,
  Twitter,
  Youtube,
  User,
} from 'lucide-react';

interface BioPreviewProps {
  avatarUrl: string;
  displayName: string;
  username: string;
  bio: string;
  facebookUrl: string;
  instagramUrl: string;
  linkedinUrl: string;
  twitterUrl: string;
  youtubeUrl: string;
}

export function BioPreview({
  avatarUrl,
  displayName,
  username,
  bio,
  facebookUrl,
  instagramUrl,
  linkedinUrl,
  twitterUrl,
  youtubeUrl,
}: BioPreviewProps) {
  const socialLinks = [
    {
      name: 'Facebook',
      url: facebookUrl,
      icon: Facebook,
      // iconColor: 'text-blue-600',
    },
    {
      name: 'Instagram',
      url: instagramUrl,
      icon: Instagram,
      // iconColor: 'text-pink-500',
    },
    {
      name: 'LinkedIn',
      url: linkedinUrl,
      icon: Linkedin,
      // iconColor: 'text-blue-700',
    },
    {
      name: 'Twitter',
      url: twitterUrl,
      icon: Twitter,
      // iconColor: 'text-black',
    },
    {
      name: 'YouTube',
      url: youtubeUrl,
      icon: Youtube,
      // iconColor: 'text-red-600',
    },
  ].filter((link) => link.url && link.url.trim() !== '');

  return (
    <div className="bg-transparent">
      {/* Preview Header */}
      <div className="flex items-center gap-2 mb-3">
        {/* <div className="w-1.5 h-1.5 bg-green-500 rounded-full animate-pulse"></div> */}
        <span className="text-[12px] font-medium text-gray-500 max-w-[150px] mx-auto">
          Live Preview
        </span>
      </div>

      {/* iPhone Mockup */}
      <div className="flex justify-center">
        <div
          className="relative bg-black rounded-[1.8rem] p-1 shadow-xl"
          style={{ width: '200px' }}
        >
          {/* Screen */}
          <div className="bg-white rounded-[1.6rem] overflow-hidden relative">
            {/* Dynamic Island */}
            <div className="absolute top-1.5 left-1/2 -translate-x-1/2 w-14 h-4 bg-black rounded-full z-10"></div>

            {/* Content */}
            <div className="pt-8 px-3 pb-5 min-h-[400px] flex flex-col items-center">
              {/* Avatar */}
              <div className="mb-3">
                {avatarUrl ? (
                  <img
                    src={avatarUrl}
                    alt={displayName || username}
                    className="w-16 h-16 rounded-full object-cover border-2 border-gray-100 shadow-md"
                  />
                ) : (
                  <div className="w-16 h-16 bg-gradient-to-br from-[#2EE6A6] to-[#1FD695] rounded-full flex items-center justify-center shadow-md">
                    <User size={26} className="text-white" />
                  </div>
                )}
              </div>

              {/* Name */}
              <h2 className="text-gray-900 font-bold text-[13px] text-center leading-tight">
                {displayName || 'Your Name'}
              </h2>

              {/* Username */}
              <p className="text-gray-400 text-[9px] text-center mt-1">
                @{username || 'username'}
              </p>

              {/* Social Icons
              {socialLinks.length > 0 && (
                <div className="flex items-center justify-center gap-2 mt-2">
                  {socialLinks.map((link, index) => {
                    const Icon = link.icon;

                    return (
                      <a
                        key={index}
                        href={link.url}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="w-5 h-5 rounded-xl border border-gray-200 bg-white flex items-center justify-center shadow-sm hover:scale-105 transition-all duration-200"
                      >
                        <Icon
                          size={12}
                          // className={link}
                        />
                      </a>
                    );
                  })}
                </div>
              )} */}

              {/* Bio */}
              {bio && (
                <p className="text-gray-500 text-[9px] text-center max-w-[150px] mx-auto mt-2 leading-relaxed">
                  {bio}
                </p>
              )}

              {/* Social Icons */}
              {socialLinks.length > 0 && (
                <div className="flex items-center justify-center gap-2 mt-2">
                  {socialLinks.map((link, index) => {
                    const Icon = link.icon;

                    return (
                      <a
                        key={index}
                        href={link.url}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="w-5 h-5 rounded-xl border border-gray-200 bg-white flex items-center justify-center shadow-sm hover:scale-105 transition-all duration-200"
                      >
                        <Icon
                          size={12}
                          // className={link}
                        />
                      </a>
                    );
                  })}
                </div>
              )}


            </div>

            {/* Home Bar */}
            <div className="absolute bottom-1 left-1/2 -translate-x-1/2 w-16 h-1 bg-gray-300 rounded-full"></div>
          </div>
        </div>
      </div>
    </div>
  );
}